...
(function () {
  ...
  /* ── ⑥ MAS review notice (restore App Store href when approved) ── */
  (function () {
    var links = document.querySelectorAll('.mas-download');
    if (!links.length) return;

    var overlay = document.createElement('div');
    overlay.className = 'mas-review-modal';
    overlay.setAttribute('role', 'dialog');
    overlay.setAttribute('aria-modal', 'true');
    overlay.setAttribute('aria-labelledby', 'mas-review-title');
    overlay.innerHTML = [
      '<div class="mas-review-dialog">',
      '  <h2 id="mas-review-title" class="mas-review-title">Almost there</h2>',
      '  <p class="mas-review-text">',
      '    MinkNote is still in review, please come back soon, will post on ',
      '    <a href="https://x.com/MinkNoteApp" target="_blank" rel="noopener">@MinkNoteApp</a>',
      '    when it&rsquo;s finally approved.',
      '  </p>',
      '  <button type="button" class="btn btn-primary mas-review-dismiss">OK</button>',
      '</div>'
    ].join('');
    document.body.appendChild(overlay);

    var dismissBtn = overlay.querySelector('.mas-review-dismiss');

    function openModal() {
      overlay.classList.add('is-open');
      document.body.classList.add('mas-review-open');
      dismissBtn.focus();
    }

    function closeModal() {
      overlay.classList.remove('is-open');
      document.body.classList.remove('mas-review-open');
    }
